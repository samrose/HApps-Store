...
pub fn handle_get_dna(app_hash: Address) -> ZomeApiResult<happs::DnaBundle> {
    Ok(utils::get_links_and_load_type::<_, happs::DnaBundle>(&app_hash, "has")?
        .first()
        .ok_or(ZomeApiError::Internal("No DNA bundles linked to app address".into()))?
        .entry.clone())
}

pub fn handle_get_ui(app_hash: Address) -> ZomeApiResult<happs::UiBundle> {
    Ok(utils::get_links_and_load_type::<_, happs::UiBundle>(&app_hash, "has")?
        .first()
