...
module.exports = (scenario) => {
  ...
  scenario('Create an app, add dna+ui bundles and retrieve', async (s, t, {liza}) => {
    ...
    const get_app_result = await liza.app.call('happs', "get_app", {app_hash:app_address})
    console.log(get_app_result)
    const app_details = get_app_result.Ok
    t.equal(app_details.uuid, App1.uuid)

    const get_all_apps_result = await liza.app.call('happs', "get_all_apps", {})
    console.log(get_all_apps_result)
    const all_app_details = get_all_apps_result.Ok
    t.equal(all_app_details.length , 1)

    const upvote_result = await liza.app.call('happs', 'upvote_app', {app_address: app_address})
    console.log(upvote_result)
    t.notEqual(upvote_result.Ok, undefined)

    const get_all_apps_result_after_upvote = await liza.app.call('happs', "get_all_apps", {})
