...
module.exports = (scenario) => {
  ...
  scenario('add Category ', async (s, t) => {
    ...
    const create_result2 = await liza.call( "app", "happs", "create_app", App2);
    console.log(create_result2)
    const app_address2 = create_result2.Ok
    t.equal(app_address2.length, 46)

    const result1 = await liza.call( "app", "happs", "add_app_to_category", {app_address: app_address, category: "good apps"})
    console.log(result1)
    t.equal(result1.Ok, null)

    const result2 = await liza.call( "app", "happs", "add_app_to_category", {app_address: app_address2, category: "good apps"})
    console.log(result2)
    t.equal(result2.Ok, null)

    const result3 = await liza.call( "app", "happs", "get_apps_by_category", {category:"good apps"})
    console.log(result3)
